...
INSERT INTO youtube_channels (channel_id, channel_name, category, subscribers,
avg_views, nox_score)
VALUES (1, 'MrBeast', 'Entertainment', '370M', '187.67M', 0.5),
...
FOREIGN KEY (channel_id) REFERENCES youtube_channels(channel_id) ON
DELETE CASCADE
);
-- Insert sample data for channel_revenue
INSERT INTO channel_revenue (channel_id, monthly_revenue, yearly_revenue)
VALUES (1, 5000000.00, 60000000.00),
...
-- Insert sample data for channel_video
INSERT INTO channel_videos (channel_id, video_title, upload_date, views, likes,
comments)
VALUES (1, 'Epic Challenge Video', '2024-03-01', 15000000, 500000, 10000),
...
INSERT INTO channel_engagement (channel_id, likes_per_video,
comments_per_video, shares_per_video) 
VALUES (1, 450000.50, 12000.75, 30000.25),
...
-- QUESTIONS
-- 1️.Get the top 5 most subscribed YouTube channels ?
SELECT channel_name, subscribers
FROM youtube_channels
ORDER BY subscribers DESC
LIMIT 5; 

-- 2️.Find the total yearly revenue of all channels ?
SELECT yt.channel_name, cr.yearly_revenue AS revenue
FROM youtube_channels AS yt
JOIN channel_revenue AS cr
ON yt.channel_id = cr.channel_id;

-- 3️.Find the video uploaded by 'MrBeast' ?
SELECT yt.channel_name, cv.video_title
FROM youtube_channels AS yt
JOIN channel_videos AS cv
ON yt.channel_id = cv.channel_id
WHERE yt.channel_name LIKE 'MrBeast';

-- 4️.Get the average engagement (likes & comments) per video for each channel 
SELECT yt.channel_name, ROUND(AVG(ce.likes_per_video),2) AS avg_likes, ROUND(AVG(ce.comments_per_video),2) AS avg_cmmnts
FROM youtube_channels AS yt
JOIN channel_engagement AS ce
ON yt.channel_id = ce.channel_id
GROUP BY yt.channel_name;

-- 5️.List channels that have a NOX score higher than 10 ?
SELECT channel_name, nox_score
FROM youtube_channels
WHERE nox_score > 10
ORDER BY nox_score DESC;

-- 6️.Find the most commented video ?
...
SELECT category, COUNT(channel_name) AS no_of_channel
FROM youtube_channels
GROUP BY category;
